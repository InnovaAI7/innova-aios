...
Usage:
    ...
    python scripts/collect.py --sources trello,calendly # Run specific ones
...
def main():
    ...
    if not available:
        print(f"[{timestamp}] No collectors found.", file=sys.stderr)
        sys.exit(1)
    ...
    conn = init_db()
    print(f"[{timestamp}] Collection started — {len(names)} sources for date={today}", file=sys.stderr)
